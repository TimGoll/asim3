...
do_rerun = true; % Da hier die Simulation nacheinander ausgeführt wird muss "true" da stehen.
...
paw_default({time}, {alpha1}, {'default'}, 'time [s]', '\alpha_{1} [°]', task_name, "alpha1"+string(choose), "plots", false, true);
paw_default({time}, {alpha2}, {'default'}, 'time [s]', '\alpha_{2} [°]', task_name, "alpha2"+string(choose), "plots", false, true);
...
% paw_default({time,time}, {a1_cache,a2_cache}, {'\alpha_{1}', '\alpha_{2}'}, 'time [s]', 'angle [°]', task_name, "alpha1_alpha2", "plots", true, true);
% while true
%     i = i+1;
%     if (time(i) < nextstep)
%         continue;
